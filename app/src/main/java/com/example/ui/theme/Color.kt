package com.example.ui.theme

import androidx.compose.ui.graphics.Color

val OceanBlue = Color(0xFF0284C7)
val OceanBlueDark = Color(0xFF0369A1)
val CoralOrange = Color(0xFFF97316)
val AmberGold = Color(0xFFF59E0B)
val EmeraldGreen = Color(0xFF10B981)
val SlateGray = Color(0xFF64748B)

val BentoSlate900 = Color(0xFF0F172A)
val BentoSlate800 = Color(0xFF1E293B)
val BentoSlate700 = Color(0xFF334155)
val BentoSlate600 = Color(0xFF475569)
val BentoSlate500 = Color(0xFF64748B)
val BentoSlate400 = Color(0xFF94A3B8)
val BentoSlate300 = Color(0xFFCBD5E1)
val BentoSlate200 = Color(0xFFE2E8F0)
val BentoSlate100 = Color(0xFFF1F5F9)
val BentoSlate50 = Color(0xFFF8FAFC)

val BentoPrimaryBlue = Color(0xFF0284C7)
val BentoAmberBg = Color(0xFFFEF3C7)
val BentoAmberText = Color(0xFFD97706)
val BentoGreenBg = Color(0xFFD1FAE5)
val BentoGreenText = Color(0xFF059669)
val BentoRedBg = Color(0xFFFEE2E2)
val BentoRedText = Color(0xFFDC2626)
val BentoBlueLight = Color(0xFFE0F2FE)
val BentoBluePill = Color(0xFFE0F2FE)
val BentoBlueText = Color(0xFF0369A1)
val BentoBorder = Color(0xFFE2E8F0)
val BentoCardWhite = Color(0xFFFFFFFF)
val BentoCanvas = Color(0xFFF8FAFC)
val BentoGold = Color(0xFFF59E0B)
val TideIncoming = Color(0xFF0284C7)
val TideReceding = Color(0xFF10B981)
